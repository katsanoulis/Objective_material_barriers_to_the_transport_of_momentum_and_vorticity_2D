...
function [FTLEm,PRAm] = ComputeInstantaneousActiveMomentumFTLE_PRA(xi_span,yi_span,tSteps,sFinal,xmax,ymax,numX,numY)
%% Load grid vectors
...
Lomega = omega(wrapTo2Pi(xgrid_over),wrapTo2Pi(ygrid_over));

Lomega = [Lomega Lomega(:,1)];
Lomega = [Lomega; Lomega(1,:)];

%% Compute interpolants for the instantaneous momentum barrier field
...
F11 = squeeze((xit(tc,:,:,1)-xit(tc,:,:,3)))/(2*rhox);
F12 = squeeze((xit(tc,:,:,2)-xit(tc,:,:,4)))/(2*rhox);
F21 = squeeze((yit(tc,:,:,1)-yit(tc,:,:,3)))/(2*rhoy);
F22 = squeeze((yit(tc,:,:,2)-yit(tc,:,:,4)))/(2*rhoy);
