...
if strcmpi(mode,'maxima')
    interp_obj = griddedInterpolant({x,y}, permute(VMatrix,[2,1]),'linear','none');
...
isMax = islocalmax(VMatrix,1) & islocalmax(VMatrix,2); % combine maxima along first dim with second dim
Xc = xi(isMax); Yc =  yi(isMax);
Nc = numel(Xc);
